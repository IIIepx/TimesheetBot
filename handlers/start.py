...
async def change_project(message: types.Message, state: FSMContext):
    ...
    await message.answer("Выберите актуальный проект.", reply_markup=keyboards.make_keyboard(tools.get_list_of_projects()))
...
async def claim_accept(
    ...
    values = [(msg.from_user.id, msg.from_user.first_name, msg.from_user.last_name, message.text.split()[-1])] 
    db.insert("users", columns, values)
    await msg.answer(
        f"{message.from_user.first_name} {message.from_user.last_name} "
        f"добавил вас в список пользователей.\n"
        f"Теперь необходимо выбрать проект",
        reply_markup=keyboards.make_keyboard(tools.get_list_of_projects()),
    )
    state_query = dp.fsm.resolve_context(
...
@router.message(UserState.project_choice, F.text.in_(tools.get_list_of_projects()))
...
async def wrong_choice(message: types.Message):
    print(tools.get_list_of_projects())
